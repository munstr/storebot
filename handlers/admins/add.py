...
from states.states import ProductState, CategoryState
from aiogram.dispatcher.filters.builtin import Command, Text
from aiogram.types.chat import ChatActions
from loader import dp, db, bot
from filters.checkUser import IsAdmin
from hashlib import md5
from handlers.users.start import settings
...
async def category_callback_handler(call: CallbackQuery, callback_data: dict, state: FSMContext):
   ...
   await show_products(call.message, products, category_idx)
...
@dp.message_handler(IsAdmin(), lambda message: message.text.isdigit(), content_types=ContentType.TEXT, state=ProductState.price)
async def process_source_to_game(message: Message, state: FSMContext):
    async with state.proxy() as data:
        data['price'] = message.text
        await message.answer("Ссылка на игру?")
        await ProductState.source.set()



@dp.message_handler(IsAdmin(), Text(startswith=["https://", "http://"]), state=ProductState.source)
...
@dp.message_handler(IsAdmin(), content_types=ContentType.TEXT, state=ProductState.source)
async def process_source_to_game_err(message: Message):
...
async def process_confirm(message: Message, state: FSMContext):
   ...
   async with state.proxy() as data:
       ...
       price = data['price']
       source = data['source']
       next_id = db.fetchone("SELECT COUNT(title) from products")[0] + 1
       ...
       db.query('INSERT INTO products VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                (idx, title, body, image, int(price), tag, next_id, source))
...
async def show_products(m, products, category_idx):
   ...
   for idx, title, body, image, price, tag, next_id, source in products:
